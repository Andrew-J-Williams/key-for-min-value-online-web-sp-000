...
def key_for_min_value(name_hash)
  ...
  min_value = 0 # If the hash is empty, we simply return the value of 'nil'.

  name_hash.each do |name, age| # Our each method will iterate over both the keys and values found within.
    if min_value == 0 || age < min_value # We create an 'or' conditional: If the value is = 'nil', we set that key equal to 'nil' as well.
